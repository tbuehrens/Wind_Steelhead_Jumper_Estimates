...
packages<-c("tidyverse","rstan","ggplot2","kableExtra")
...
N_aug%>%
  dplyr::rename(spawn_year = year)%>%
  kbl(caption = "Table 2.The smoothed  & observed August Mark-Resight Abundance of Wild Wind River Steelhead",digits =3)%>%
  kable_classic(full_width = F, html_font = "Cambria")
...
N_sept%>%
  dplyr::rename(spawn_year = year)%>%
  kbl(caption = "Table 3.The smoothed & observed September Mark-Resight Abundance of Wild Wind River Steelhead",digits =3)%>%
  kable_classic(full_width = F, html_font = "Cambria")
...
hier_jr<-quantile(1/results$hier_p_tr,c(0.025,0.25,0.5,0.75,0.975))%>%
  as.data.frame()%>%
  rownames_to_column()%>%
  dplyr::rename(quant="rowname",hier_jr=".")%>%
  as_tibble()

hier_jr%>%
  kbl(caption = "Table 4.The hierarchical jumper rate" ,digits =3)%>%
  kable_classic(full_width = F, html_font = "Cambria")
```
